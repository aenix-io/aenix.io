// Service worker для статик-демо на GitHub Pages: перехватывает /kc (Keycloak
// Account API) и отдаёт canned-ответы. Мутации (сохранение профиля, отзыв
// сессии) принимаются и отбрасываются — стор демо всё равно сбрасывается.
...
self.addEventListener("fetch", (e) => {
  ...
  const m = url.pathname.match(/\/kc(\/.*|)$/);
  if (!m) return; // не /kc — пропускаем
  const suffix = m[1] || "";
  const json = (o) => new Response(JSON.stringify(o), { headers: { "content-type": "application/json" } });
  if (e.request.method !== "GET") { e.respondWith(json({})); return; }
  const body = Object.prototype.hasOwnProperty.call(CANNED, suffix) ? CANNED[suffix] : CANNED[""];
  e.respondWith(json(body ?? {}));
});
