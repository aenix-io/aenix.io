...
проверь. Здесь иначе: вы описываете, **как должно быть**, а система сама решает, что для
этого сделать.
...
`kubectl get tenants` работает, хотя в Kubernetes никаких тенантов нет. Дело в **CRD** —
...
выполняет **оператор** — программа, которая следит за объектами своего типа и приводит мир
в соответствие.
...
Что это даёт на практике: у любого изменения есть автор и время, откат — это возврат к
прошлому состоянию файла, а правка, сделанная руками в обход Git, будет отменена при
следующей сверке. Последнее раздражает ровно до первого случая, когда чужая ручная правка
что-нибудь роняет.
...
чарт, и заказ сервиса разворачивает именно его.
...
объект → оператор → HelmRelease → чарт → работающие поды
</p>
...
<li>Что ручная правка в обход Git будет отменена при следующей сверке.</li>
<li>Что чарт — единица упаковки, и каждая позиция каталога это чарт.</li>
<li>Цепочку: объект → оператор → HelmRelease → чарт → поды.</li>
</ul>
</div>
