...
работает поверх** и нужен там, где требуются отдельные сети со своими диапазонами: сети
виртуальных машин и VPC тенантов. На одном поде они не пересекаются.

## Kube-OVN — сети тенантов

Cilium даёт одну плоскую сеть на кластер. Этого мало, когда тенантов много и каждому
...
<li>Что точка входа для HTTP заводится у каждого тенанта отдельно.</li>
