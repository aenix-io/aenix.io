...
quiz:
  ...
  questions:
    - q: "How many honest VMware migration paths does the article identify?"
      options:
        - { text: "Five distinct paths", correct: false }
        - { text: "Eight typical paths", correct: false }
        - { text: "Three honest paths", correct: true }
    ...
    - q: "For most 2026 migrations driven by Broadcom pricing or sovereignty, which path does the article recommend?"
      options:
        - { text: "KubeVirt-based, open destination", correct: true }
        - { text: "Vendor-tools-led VMware path", correct: false }
        - { text: "Lift-and-shift to public cloud", correct: false }
    ...
    - q: "Which Red Hat tool handles VMware → KubeVirt at bulk scale?"
      options:
        - { text: "KubeVirt CDI alone (no orchestrator)", correct: false }
        - { text: "Forklift, also packaged as MTV", correct: true }
        - { text: "virt-v2v alone (called per VM)", correct: false }
      explanation: "Forklift (also packaged as MTV — Migration Toolkit for Virtualization) handles bulk VM migration with metadata preservation. virt-v2v is the lower-level conversion tool used internally by Forklift. KubeVirt CDI handles disk-level import."
    - q: "How many workloads per cohort does the article suggest for cohort-based migration?"
      options:
        - { text: "1–5 workloads per cohort", correct: false }
        - { text: "500+ workloads per cohort", correct: false }
        - { text: "10–50 workloads per cohort", correct: true }
    ...
    - q: "What is the typical destination-platform-build duration before migration cohort 1 starts (for Cozystack-based destinations)?"
      options:
        - { text: "1 week of platform build", correct: false }
