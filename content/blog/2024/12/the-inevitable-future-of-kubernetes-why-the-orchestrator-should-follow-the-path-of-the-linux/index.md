...
quiz:
  ...
  questions:
    - q: "Whose KubeCon talk does the article respond to as the entry point?"
      options:
        - { text: "Liz Rice on eBPF performance in production clusters", correct: false }
        - { text: "Tim Hockin proposing a complexity budget for Kubernetes", correct: true }
        - { text: "Kelsey Hightower revisiting Kubernetes the Hard Way", correct: false }
    ...
    - q: "What is the central analogy of Timur's piece?"
      options:
        - { text: "Kubernetes should split into a microkernel-and-modules architecture", correct: false }
        - { text: "Kubernetes should follow the Apache HTTPd modular-server model", correct: false }
        - { text: "Kubernetes should evolve like the Linux kernel, with platforms as distros", correct: true }
    ...
    - q: "Which CNCF body is named as a candidate for initiating the strategic discussion about Kubernetes' future as a kernel-equivalent?"
      options:
        - { text: "Only the CNCF Foundation Board acting unilaterally", correct: false }
        - { text: "The Cloud Native Buildpacks maintainer group", correct: false }
        - { text: "TAG App Delivery, the TOC, maintainers, and major sponsors", correct: true }
    ...
    - q: "What does the article identify as the bottleneck under the current 'Kubernetes as standalone software' framing?"
      options:
        - { text: "CPU performance ceiling on commodity nodes", correct: false }
        - { text: "Reliance on engineers who must specialise in Kubernetes itself", correct: true }
        - { text: "Storage costs across replicated cluster state", correct: false }
